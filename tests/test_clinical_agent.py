...
class TestClinicalIntakeAgent:
    ...
    @pytest.mark.asyncio
    async def test_process_clinical_intake_success(
        self, agent_instance, sample_audio_data
    ):
        """Test successful clinical intake processing."""
        # Mock all service responses
        with patch.object(stt_service, "transcribe_audio") as mock_stt, patch.object(
            ner_service, "extract_entities"
        ) as mock_ner, patch.object(
            llm_service, "generate_clinical_summary"
        ) as mock_llm, patch.object(
            llm_service, "generate_structured_notes"
        ) as mock_structured, patch.object(
            storage_service, "save_audio_record"
        ) as mock_save_audio, patch.object(
            storage_service, "save_medical_entities"
        ) as mock_save_entities, patch.object(
            storage_service, "save_clinical_notes"
        ) as mock_save_notes:
    ...
    @pytest.mark.asyncio
    async def test_process_clinical_intake_stt_failure(
        self, agent_instance, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_clinical_intake_ner_failure(
        self, agent_instance, sample_audio_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_clinical_intake_with_translation(
        self, agent_instance, sample_audio_data
    ):
        ...
        with patch.object(stt_service, "transcribe_audio") as mock_stt, patch.object(
            ...
            translation_service, "translate_clinical_notes"
...
class TestSTTTool:
    ...
    async def test_stt_tool_execute_success(self, stt_tool):
        ...
        with patch.object(stt_service, "transcribe_audio") as mock_transcribe:
            ...
            result = await stt_tool.execute(
                audio_data=b"fake_audio", audio_format="wav", duration_seconds=2.0
            )
    ...
    async def test_stt_tool_execute_failure(self, stt_tool):
        ...
        with patch.object(stt_service, "transcribe_audio") as mock_transcribe:
            ...
            result = await stt_tool.execute(
                audio_data=b"fake_audio", audio_format="wav"
            )
...
class TestLLMTool:
    ...
    async def test_llm_tool_execute_failure(self, llm_tool):
        ...
        with patch.object(llm_service, "generate_clinical_summary") as mock_summary:
            ...
            result = await llm_tool.execute(
                transcript="Test transcript", entities=[], user_id="test_user"
            )
...
class TestTranslationTool:
    ...
    async def test_translation_tool_execute_success(self, translation_tool):
        ...
        with patch.object(
            ...
            result = await translation_tool.execute(
                clinical_notes={"hauptbeschwerden": ["Kopfschmerzen"]}, target_lang="en"
            )
    ...
    async def test_translation_tool_execute_failure(self, translation_tool):
        ...
        with patch.object(
            ...
            result = await translation_tool.execute(
                clinical_notes={"test": "data"}, target_lang="en"
            )
...
class TestStorageTool:
    ...
    async def test_storage_tool_execute_unknown_operation(self, storage_tool):
        ...
        result = await storage_tool.execute(operation="unknown_operation", data={})
        ...
        with patch.object(stt_service, "transcribe_audio") as mock_stt, patch.object(
            ...
            mock_ner.return_value = [
                ...
                MagicMock(
                    ...
                    confidence=0.85,
                    icd_code="H81",
                    icd_description="Schwindel",
                    normalized_text="schwindel",
                    category="symptom",
                ),
            ]
