...
class TestSTTService:
    ...
    def sample_audio_data(self):
        ...
        audio.write(b'RIFF\x24\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x40\x1f\x00\x00\x80\x3e\x00\x00\x02\x00\x10\x00data\x00\x00\x00\x00')
        audio.seek(0)
        return audio.getvalue()
    
    @pytest.mark.asyncio
    async def test_transcribe_audio_success_faster_whisper(self, stt_service_instance, sample_audio_data):
        """Test successful transcription with Faster-Whisper."""
        # Mock successful Faster-Whisper response
        mock_segments = [MagicMock(text="Der Patient klagt über Kopfschmerzen und Schwindel.")]
        mock_info = MagicMock()
        
        with patch.object(stt_service_instance, 'fw_model') as mock_model:
            mock_model.transcribe.return_value = (mock_segments, mock_info)
            result = await stt_service_instance.transcribe_audio(sample_audio_data, "wav")
            
            assert result["text"] == "Der Patient klagt über Kopfschmerzen und Schwindel."
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_fallback_to_together(self, stt_service_instance, sample_audio_data):
        """Test fallback to Together AI when Faster-Whisper fails."""
        # Mock Faster-Whisper failure
        with patch.object(stt_service_instance, '_transcribe_with_whisper', side_effect=Exception("Faster-Whisper failed")):
            # Mock successful Together AI response
            mock_response = MagicMock()
            mock_response.text = "Patient complains of headache and dizziness."
            
            with patch.object(stt_service_instance.together_client.audio, 'transcribe', return_value=mock_response):
                result = await stt_service_instance.transcribe_audio(sample_audio_data, "wav")
    ...
    async def test_transcribe_audio_validation(self, stt_service_instance):
        ...
        with pytest.raises(ValueError, match="Audio too short"):
            await stt_service_instance.transcribe_audio(b"", "wav", duration_seconds=0.05)
        
        # Test audio too long
        long_audio = b"x" * 1000
        with pytest.raises(ValueError, match="Audio too long"):
            await stt_service_instance.transcribe_audio(long_audio, "wav", duration_seconds=400)
    ...
    async def test_transcribe_streaming(self, stt_service_instance, sample_audio_data):
        ...
        with patch.object(stt_service_instance, 'fw_model') as mock_model:
            ...
            results = []
            async for result in stt_service_instance.transcribe_streaming(audio_chunks()):
    ...
    async def test_health_check(self, stt_service_instance):
        ...
        with patch.object(stt_service_instance, 'fw_model') as mock_model, \
             patch.object(stt_service_instance.together_client.audio, 'transcribe', return_value=mock_together_response):
            mock_model.transcribe.return_value = (mock_segments, mock_info)
    ...
    @pytest.mark.asyncio
    async def test_latency_threshold_trigger(self, stt_service_instance, sample_audio_data):
        ...
        with patch.object(stt_service_instance, '_transcribe_with_whisper', side_effect=slow_fw_transcribe), \
             patch.object(stt_service_instance.together_client.audio, 'transcribe', return_value=together_mock_response):
