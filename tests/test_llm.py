...
class TestLLMService:
    ...
    @pytest.mark.asyncio
    async def test_generate_clinical_summary_success_mistral(self, llm_service_instance, sample_transcript, sample_entities):
        """Test successful clinical summary generation with Mistral."""
        # Mock successful Mistral response
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "choices": [{"message": {"content": "Strukturierte klinische Zusammenfassung..."}}],
            "usage": {"total_tokens": 150}
        }
        mock_response.raise_for_status.return_value = None
        
        with patch.object(llm_service_instance.mistral_client, 'post', return_value=mock_response):
    ...
    @pytest.mark.asyncio
    async def test_generate_clinical_summary_fallback_openrouter(self, llm_service_instance, sample_transcript, sample_entities):
        ...
        openrouter_mock_response.json.return_value = {
            "choices": [{"message": {"content": "Clinical summary from fallback model..."}}],
            "usage": {"total_tokens": 120}
        }
        openrouter_mock_response.raise_for_status.return_value = None
        
        with patch.object(llm_service_instance.mistral_client, 'post', return_value=mistral_mock_response), \
             patch.object(llm_service_instance.openrouter_client, 'post', return_value=openrouter_mock_response):
    ...
    @pytest.mark.asyncio
    async def test_generate_clinical_summary_all_providers_fail(self, llm_service_instance, sample_transcript, sample_entities):
        ...
        with patch.object(llm_service_instance.mistral_client, 'post', return_value=mock_response), \
             patch.object(llm_service_instance.openrouter_client, 'post', return_value=mock_response), \
             patch.object(llm_service_instance.together_client, 'post', return_value=mock_response):
    ...
    async def test_generate_structured_notes_success(self, llm_service_instance):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "choices": [{"message": {"content": '{"hauptbeschwerden": ["Kopfschmerzen", "Schwindel"], "aktuelle_symptome": ["Übelkeit", "Müdigkeit"], "medikamente": ["Metformin", "Aspirin"]}'}}],
            "usage": {"total_tokens": 100}
        }
        mock_response.raise_for_status.return_value = None
        
        with patch.object(llm_service_instance.openrouter_client, 'post', return_value=mock_response):
    ...
    @pytest.mark.asyncio
    async def test_generate_structured_notes_json_parsing_failure(self, llm_service_instance):
    ...
    def test_format_entities_for_llm(self, llm_service_instance):
        ...
        entities = [
            {"text": "Kopfschmerzen", "label": "SYMPTOM", "icd_code": "R51", "category": "symptom"},
            {"text": "Diabetes", "label": "DIAGNOSIS", "icd_code": "E11", "category": "diagnosis"}
        ]
    ...
    @pytest.mark.asyncio
    async def test_temperature_zero_enforcement(self, llm_service_instance, sample_transcript, sample_entities):
        ...
        with patch.object(llm_service_instance.mistral_client, 'post', return_value=mock_response) as mock_post:
            await llm_service_instance.generate_clinical_summary(
                transcript=sample_transcript,
                entities=sample_entities,
                user_id="test_user"
