...
def render_report_pdf(report: ClinicalReport) -> Tuple[str, bytes]:
    ...
    logger.info(
        "Starting PDF rendering", extra={"extra_fields": {"html_length": len(html)}}
    )
    ...
    if pdf_bytes is not None:
        logger.info(
            "WeasyPrint succeeded", extra={"extra_fields": {"pdf_size": len(pdf_bytes)}}
        )
    else:
        ...
        if pdf_bytes is not None:
            logger.info(
                "ReportLab succeeded",
                extra={"extra_fields": {"pdf_size": len(pdf_bytes)}},
            )
    if pdf_bytes is None:
        logger.warning(
            "Both ReportLab and WeasyPrint failed, trying builtin PDF fallback"
        )
...
def _render_with_reportlab(html: str) -> Optional[bytes]:
    ...
    logger.info(
        "ReportLab rendered successfully",
        extra={"extra_fields": {"pdf_size": len(pdf_bytes)}},
    )
    return pdf_bytes
    ...
    for line in lines:
        ...
        # Handle checkbox items
            ...
            if checkbox_text:
                ...
                elements.append(
                    {"type": "line", "text": f"{checkbox_symbol} {checkbox_text}"}
                )
        # Handle div content (general)
            ...
            if (
                content
                and not content.startswith("<!DOCTYPE")
                and not content.startswith("<html")
            ):
