...
class STTService:
    ...
    async def warm_up_models(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Faster-Whisper model loaded ({model_size}, {device}, {compute_type})"
                )
            except Exception as e:
    ...
    async def _process_audio_for_stt(
        ...
        try:
            ...
            return await audio_processor.process_audio_for_stt(
                audio_data, target_format
            )
        ...
        try:
            ...
            if not use_fallback and self.together_client:
                try:
                    ...
                    if latency_ms > settings.stt_final_threshold:
                        logger.warning(
                            f"Fallback STT latency {latency_ms:.1f}ms exceeds threshold {settings.stt_final_threshold}ms"
                        )
            ...
            if self.fw_model:
                try:
                    ...
                    if latency_ms > settings.stt_final_threshold:
                        logger.warning(
                            f"Primary STT latency {latency_ms:.1f}ms exceeds threshold {settings.stt_final_threshold}ms, trying fallback"
                        )
                        use_fallback = True
                    else:
                        self._update_stats(result["latency"], True)
    ...
    async def transcribe_streaming(
        self, audio_chunks: AsyncGenerator[bytes, None], chunk_duration: float = 1.5
        ...
        async for chunk in audio_chunks:
            try:
                ...
                try:
                    ...
                    if current_text != last_transcription and len(current_text) > len(
                        last_transcription
                    ):
                        yield {
                            "type": "partial",
